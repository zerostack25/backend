...
async function analyzeFlood(events) {
    const prompt = `Kamu adalah AI internal backend anti-flood. Kamu berjalan di sistem Mayzaa API. Analisa log berikut dan jawab JSON saja: {"flood":true/false,"reason":"..."}\n\n${JSON.stringify(events.slice(0, 20))}`;
    const answer = await askGemini(prompt);
    try {
        return JSON.parse(answer.match(/\{[\s\S]*\}/)?.[0] || answer);
    } catch (e) {
        return { flood: false, reason: answer || 'AI parse failed' };
    }
}

module.exports = { askGemini, analyzeFlood };